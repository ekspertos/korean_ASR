...
class MaskAlongAxis(torch.nn.Module):
    def __init__(
        ...
        num_mask: int = 2,
        size_ratio: float = 0.04,
        multiplicity_ratio: float = 0.04,
        ...
        self.multiplicity_ratio = multiplicity_ratio
    ...
    def forward(self, spec: torch.Tensor, spec_lengths: torch.Tensor = None):
        ...
        if self.mask_axis == "time":
            self.num_mask = int(min(20, spec_lengths[0] * self.multiplicity_ratio))
            self.mask_width_range[1] = int(spec_lengths[0] * self.size_ratio)

        #print("mask_axis:", self.mask_axis)
        #print("num_mask:", self.num_mask)
        #print("mask_width_range:", self.mask_width_range)

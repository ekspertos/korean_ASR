...
def repl_symbol(msg):
    msg = msg.group(0)
    msg = msg[4:-1]
    msg = regex.sub(r'\((?:[^()]++|(?0))++\)', repl_symbol, msg)
    return msg
...
def main(args):
    ...
    logging.debug("stage 6: Remove symbols related to transcription rules(Korean free speech)")
    in_fn = "5_remove_error_punctuation_marks.txt"
    out_fn = "6_remove_transcript_rule_symbols.txt"
    if os.path.exists(args.log_dir + "/" + in_fn) and args.stage <= 6:
        logging.debug(">> reading %s", args.log_dir + "/" + in_fn)
        with codecs.open(args.log_dir + "/" + in_fn, mode="r", encoding="utf-8") as f:
            trans_rm_repeat_sym = f.readlines()
        ...
        for ix, _line in enumerate(trans_rm_repeat_sym):
            ...
            # remove error punctuation marks
            line = regex.sub(r'\((?:[^()]++|(?0))++\)', repl_symbol, trans)
    ...
    shutil.copy(args.log_dir + "/" + "6_remove_transcript_rule_symbols.txt", out_fn)
